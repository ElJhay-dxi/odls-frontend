...
export default function HydrologyPage() {
  ...
  useEffect(() => {
    if (autoPlantCode) { setPlantCode(autoPlantCode); setFilterPlant(autoPlantCode); }
  }, [autoPlantCode]);
  ...
  const handleSave = async () => {
    ...
    try {
      if (tabIndex === 0) {
        const payload = { headWaterLevel: Number(headWaterLevel), tailWaterLevel: Number(tailWaterLevel) };
        if (editTarget) await hydroWaterLevelApi.update(editTarget.id, payload);
        else await hydroWaterLevelApi.create({ plantCode, logDate, ...payload });
      } else {
        const payload = { unitDischarge: Number(unitDischarge), spillwayDischarge: Number(spillwayDischarge), efficiencyKwCfs: Number(effKwCfs), efficiencyKwMcs: Number(effKwMcs) };
        if (editTarget) await hydroWaterDischargeApi.update(editTarget.id, payload);
        else await hydroWaterDischargeApi.create({ plantCode, logDate, ...payload });
      }
      setSaveSuccess(true); resetForm(); fetchRecords();
    } catch (err: unknown) {
  ...
  // ── Plant type guard — blocks access for thermal-only users ──────────────────
  if (isWrongPlantType) return (
    <Box>
      <PageHeader title="Hydrology" subtitle="Daily hydro water levels and discharge readings"
  ...
  return (
    <Box>
      <PageHeader title="Hydrology" subtitle="Daily hydro water levels and discharge readings — Hydro plants only"
        breadcrumbs={[{ label: 'Hydrology' }]} />
            ...
            <CardContent>
              ...
              <Stack spacing={2.5}>
                <Grid container spacing={2}>
                  ...
                  <Grid size={{ xs: 12, sm: 6 }}>
                    <TextField label="Log Date" type="date" fullWidth required disabled={!!editTarget}
                      value={logDate} onChange={(e) => setLogDate(e.target.value)} slotProps={{ inputLabel: { shrink: true } }} />
                  </Grid>
                </Grid>
                ...
                {tabIndex === 1 && (
                  <Grid container spacing={2}>
                    <Grid size={{ xs: 12, sm: 6 }}>
                      <TextField label="Unit Discharge" type="number" fullWidth required value={unitDischarge} onChange={(e) => setUnitDischarge(e.target.value)} />
                    </Grid>
                    <Grid size={{ xs: 12, sm: 6 }}>
                      <TextField label="Spillway Discharge" type="number" fullWidth required value={spillwayDischarge} onChange={(e) => setSpillwayDischarge(e.target.value)} />
                    </Grid>
                    <Grid size={{ xs: 12, sm: 6 }}>
                      <TextField label="Efficiency (kW/cfs)" type="number" fullWidth required value={effKwCfs} onChange={(e) => setEffKwCfs(e.target.value)} />
                    </Grid>
                    <Grid size={{ xs: 12, sm: 6 }}>
                      <TextField label="Efficiency (kW/mcs)" type="number" fullWidth required value={effKwMcs} onChange={(e) => setEffKwMcs(e.target.value)} />
                    </Grid>
                  </Grid>
                )}
            ...
            <CardContent>
              <Stack spacing={1.5} sx={{ mb: 2 }}>
                ...
                <Stack direction="row" spacing={1}>
                  <TextField label="Date" type="date" size="small" value={filterDate} onChange={(e) => setFilterDate(e.target.value)} sx={{ flex: 1 }} slotProps={{ inputLabel: { shrink: true } }} />
                ...
                levelRecords.length === 0 && !loadingRecords ? (
                  ...
                  <TableContainer sx={{ maxHeight: 480 }}>
                    <Table size="small" stickyHeader>
                      <TableHead>
                        <TableRow>
                          <TableCell>Date</TableCell><TableCell>Head (m)</TableCell><TableCell>Tail (m)</TableCell><TableCell>Net Head (m)</TableCell><TableCell align="right">Actions</TableCell>
                        </TableRow>
                      </TableHead>
                      <TableBody>
                        {levelRecords.map((row) => (
                          <TableRow key={row.id} hover>
                            ...
                            <TableCell><Chip label={row.netHead.toFixed(3)} size="small" color="info" variant="outlined" sx={{ fontWeight: 700 }} /></TableCell>
                            <TableCell align="right">
                              {canEdit && <Tooltip title="Edit"><IconButton size="small" color="primary" onClick={() => openEditLevel(row)}><Edit fontSize="small" /></IconButton></Tooltip>}
                              {canDelete && <Tooltip title="Delete"><IconButton size="small" color="error" onClick={() => setDeleteTarget({ id: row.id, label: `${row.plantCode} levels on ${row.logDate.split('T')[0]}` })}><Delete fontSize="small" /></IconButton></Tooltip>}
                            </TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                ...
                dischargeRecords.length === 0 && !loadingRecords ? (
                  ...
                  <TableContainer sx={{ maxHeight: 480 }}>
                    <Table size="small" stickyHeader>
                      <TableHead>
                        <TableRow>
                          <TableCell>Date</TableCell><TableCell>Unit Discharge</TableCell><TableCell>Spillway</TableCell><TableCell>Eff. kW/cfs</TableCell><TableCell>Eff. kW/mcs</TableCell><TableCell align="right">Actions</TableCell>
                        </TableRow>
                      </TableHead>
                      <TableBody>
                        {dischargeRecords.map((row) => (
                          <TableRow key={row.id} hover>
                            ...
                            <TableCell align="right">
                              {canEdit && <Tooltip title="Edit"><IconButton size="small" color="primary" onClick={() => openEditDischarge(row)}><Edit fontSize="small" /></IconButton></Tooltip>}
                              {canDelete && <Tooltip title="Delete"><IconButton size="small" color="error" onClick={() => setDeleteTarget({ id: row.id, label: `${row.plantCode} discharge on ${row.logDate.split('T')[0]}` })}><Delete fontSize="small" /></IconButton></Tooltip>}
                            </TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
      ...
      <ConfirmDialog open={!!deleteTarget} title="Delete Reading" message={`Delete ${deleteTarget?.label}?`}
