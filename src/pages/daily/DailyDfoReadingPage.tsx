...
export default function DailyDfoReadingPage() {
  ...
  useEffect(() => {
    if (autoPlantCode) { setForm((prev) => ({ ...prev, plantCode: autoPlantCode })); setFilterPlant(autoPlantCode); }
  }, [autoPlantCode]);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editTarget) {
        await dailyDfoReadingApi.update(editTarget.id, { currentReading: Number(updateForm.currentReading) });
        cancelEdit();
      } else {
        await dailyDfoReadingApi.create({ plantCode: form.plantCode, logDate: form.logDate, previousReading: isFirstEntry ? Number(manualPreviousReading) : undefined, currentReading: Number(form.currentReading) });
  ...
  if (isWrongPlantType) return (
    <Box>
      <PageHeader title="Daily DFO Readings" subtitle="Daily Distillate Fuel Oil tank readings per plant"
  ...
  return (
    <Box>
      <PageHeader title="Daily DFO Readings"
        ...
        <Grid size={{ xs: 12, lg: 6 }}>
          <Card>
            ...
            <CardContent>
              ...
              <Paper variant="outlined" sx={{ p: 2, mb: 2.5, borderRadius: 2 }}>
                <Typography variant="caption" color="text.secondary" sx={{ mb: 1.5, display: 'block', textTransform: 'uppercase', letterSpacing: 1, fontWeight: 700 }}>Tank Readings (m³)</Typography>
                <Grid container spacing={2}>
                  <Grid size={{ xs: 12, sm: 6 }}>
                  ...
                  <Grid size={{ xs: 12, sm: 6 }}>
                    <TextField label="Current Reading" type="number" fullWidth required
                      ...
                      onChange={(e) => { if (editTarget) setUpdateForm({ currentReading: e.target.value }); else setForm((prev) => ({ ...prev, currentReading: e.target.value })); }}
