...
import { Save, Search, Edit, Delete, Bolt, History } from '@mui/icons-material';
...
export default function DailyReactivePowerPage() {
  ...
  const [records, setRecords] = useState<DailyReactivePower[]>([]);
  const [loadingRecords, setLoadingRecords] = useState(false);
  const [recordsError, setRecordsError] = useState<string | null>(null);

  const [deleteTarget, setDeleteTarget] = useState<DailyReactivePower | null>(null);
  ...
  useEffect(() => {
    ...
    dailyReactivePowerApi.getAll({ plantCode: form.plantCode })
      .then((res) => {
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      const res = await dailyReactivePowerApi.getAll({ plantCode: filterPlant });
  ...
  const openEdit = (row: DailyReactivePower) => {
    setEditTarget(row);
    setSaveError(null);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editTarget) {
        await dailyReactivePowerApi.update(editTarget.id, {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await dailyReactivePowerApi.delete(deleteTarget.id);
  ...
  return (
    <Box>
      <PageHeader
        title="Daily Reactive Power"
        subtitle="Daily MVarh reactive power readings — applies to both Hydro and Thermal plants"
        breadcrumbs={[{ label: 'Daily Readings' }, { label: 'Reactive Power' }]}
      />
              ...
              <Paper variant="outlined" sx={{ p: 2, mb: 2.5, borderRadius: 2, backgroundColor: 'action.hover' }}>
                ...
                <Grid container spacing={2}>
                  <Grid size={{ xs: 12, sm: 6 }}>
                    ...
                    <Typography variant="h6" sx={{ fontWeight: 700 }}>{previewDifference.toFixed(2)} MVarh</Typography>
                  </Grid>
                  <Grid size={{ xs: 12, sm: 6 }}>
                    <Typography variant="caption" color="text.secondary">Progressive Total</Typography>
                    <Typography variant="h6" sx={{ fontWeight: 700 }}>{previewProgressiveTotal.toFixed(2)} MVarh</Typography>
                  </Grid>
                </Grid>
              </Paper>
