...
export default function DailyEnergyGenerationHydroPage() {
  ...
  const openEdit = (row: DailyEnergyGenerationHydro) => {
    ...
    setUpdateForm({
      ...
      akosomboPeakLoadMW: row.akosomboPeakLoadMW ?? '',
      akosomboPeakLoadTime: row.akosomboPeakLoadTime?.slice(0, 5) ?? '',
    });
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editTarget) {
        await dailyEnergyGenerationHydroApi.update(editTarget.id, {
          currentReading: Number(updateForm.currentReading),
          averagePowerFactor: toNum(updateForm.averagePowerFactor),
          akosomboPeakLoadMW: toNum(updateForm.akosomboPeakLoadMW),
          akosomboPeakLoadTime: updateForm.akosomboPeakLoadTime || undefined,
        });
        cancelEdit();
      } else {
            ...
            <CardContent>
              ...
              <Paper variant="outlined" sx={{ p: 2, mb: 2.5, borderRadius: 2 }}>
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid size={{ xs: 12, sm: 6 }}>
                    <TextField
                      label="Average Power Factor" type="number" fullWidth
                      value={fv('averagePowerFactor')}
                      onChange={(e) => setField('averagePowerFactor', e.target.value)}
                    />
                  </Grid>
                </Grid>
              </Paper>
              ...
              <Paper variant="outlined" sx={{ p: 2, mb: 2.5, borderRadius: 2 }}>
                <Typography variant="caption" color="text.secondary"
                  sx={{ mb: 1.5, display: 'block', textTransform: 'uppercase', letterSpacing: 1, fontWeight: 700 }}>
                  AKOSOMBO GS Peak Load (Nexus)
                </Typography>
                <Grid container spacing={2}>
                  <Grid size={{ xs: 12, sm: 6 }}>
                    <TextField
                      label="Peak Load" type="number" fullWidth
                      value={fv('akosomboPeakLoadMW')}
                      onChange={(e) => setField('akosomboPeakLoadMW', e.target.value)}
                      slotProps={{ input: { endAdornment: <Typography variant="caption" color="text.secondary">MW</Typography> } }}
                    />
                  </Grid>
                  <Grid size={{ xs: 12, sm: 6 }}>
                    <TextField
                      label="Peak Load Time" type="time" fullWidth
                      value={fv('akosomboPeakLoadTime')}
                      onChange={(e) => setField('akosomboPeakLoadTime', e.target.value)}
                      slotProps={{ inputLabel: { shrink: true } }}
                    />
                  </Grid>
                </Grid>
