...
export default function DailyLcoReadingPage() {
  ...
  const [form, setForm] = useState<DailyLcoReadingForm>(emptyForm);
  ...
  const [records, setRecords] = useState<DailyLcoReading[]>([]);
  const [loadingRecords, setLoadingRecords] = useState(false);
  const [recordsError, setRecordsError] = useState<string | null>(null);

  const [deleteTarget, setDeleteTarget] = useState<DailyLcoReading | null>(null);
  ...
  useEffect(() => {
    ...
    dailyLcoReadingApi.getAll({ plantCode: form.plantCode })
      .then((res) => {
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      const res = await dailyLcoReadingApi.getAll({ plantCode: filterPlant });
  ...
  const handleSave = async () => {
    ...
    try {
      if (editTarget) {
        await dailyLcoReadingApi.update(editTarget.id, {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await dailyLcoReadingApi.delete(deleteTarget.id);
  ...
  return (
    <Box>
      <PageHeader
        ...
        subtitle="Daily Light Crude Oil tank readings per plant with automatic progressive totals"
